...
test -w /nix/var/nix || doas sh -c 'mkdir -pv -m 1735 /nix/var/nix && chown -Rv '"$(id -nu)":"$(id -gn)"' /nix'
...
(test -f nix || wget -O nix "https://hydra.nixos.org/build/$BUILD_ID/download-by-type/file/binary-dist") \
...
&& (getent group kvm || doas groupadd kvm) \
&& doas usermod --append --groups kvm "$USER" \
&& echo "End kvm stuff!" \
&& (test -w /dev/kvm || doas chown -v "$(id -u)":"$(id -g)" /dev/kvm) 
